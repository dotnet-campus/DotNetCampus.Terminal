...
sequenceDiagram
    ...
    CM->>JS: SaveRemoteDeviceAsync(deviceInfo)
    JS->>JS: LoadJsonConfigurationAsync()
    JS->>JS: 查找/更新/新增设备
    JS->>FS: 写入JSON文件
    FS-->>JS: 完成
    JS-->>CM: 成功
...
**原子性保存**：
```csharp
private async Task SaveJsonConfigurationAsync(DeviceConfiguration configuration)
{
    // 确保目录存在
    ...
    // 转换为 JSON 格式并保存
    var jsonContent = JsonSerializer.Serialize(configuration, ConfigurationJsonContext.Default.DeviceConfiguration);
