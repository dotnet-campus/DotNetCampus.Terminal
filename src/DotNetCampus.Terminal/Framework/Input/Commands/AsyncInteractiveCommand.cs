...
public class AsyncInteractiveCommand<TInteraction>(Func<TInteraction, Task> action)
    : CanExecuteObservableCommand<AsyncInteractiveCommand<TInteraction>>
    where TInteraction : InteractiveCommandInteraction
{
    ...
    public AsyncInteractiveCommand<TInteraction> ProvideInteraction(TInteraction interaction)
    {
...
public class AsyncInteractiveCommand<TInteraction, TParameter>(Func<TInteraction, TParameter, Task> action)
    : CanExecuteObservableCommand<AsyncInteractiveCommand<TInteraction>>
    where TInteraction : InteractiveCommandInteraction
{
    private TInteraction? _interaction;
    ...
    public async void Execute(TParameter parameter)
    {
        ...
        try
        {
            await action(_interaction!, parameter);
            ReportCompleted();
        }
        catch (Exception ex)
        {
    ...
    public override void Execute(object? parameter)
    {
        ...
        {
            throw new ArgumentException(
                $"The parameter type does not match, expected type is {typeof(TParameter)}, actual type is {parameter.GetType()}.");
        }
    }
    ...
    /// <param name="interaction">交互式命令的交互细节，请自行定义一个记录（record）继承自 <see cref="InteractiveCommandInteraction"/>。</param>
    /// <returns>构造器模式。</returns>
    public AsyncInteractiveCommand<TInteraction, TParameter> ProvideInteraction(TInteraction interaction)
    {
        _interaction = interaction;
        return this;
    }
